...
//$repository = $this->getDoctrine()->getRepository(Article::class);
    ...
    /**
     * @Route("/produit/tri", name="triNom")
     */

    public function triNom(ProduitRepository $produitRepository, PaginatorInterface $paginator, Request $request,FlashyNotifier $flashy): Response
    {
      $donnes = $produitRepository->listOrderByNom();
       $produits = $paginator->paginate(
           $donnes,
           $request->query->getInt('page', 1),
            4
    ...
    {
        $donnes = $produitRepository->listOrderByPrix();
        $produits = $paginator->paginate(
            $donnes,
            $request->query->getInt('page', 1),
            4
